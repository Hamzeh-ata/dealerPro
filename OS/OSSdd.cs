    ...
    public class OSSdd : Hub, IOSSdd
    {
        ...
        public async Task getSSD()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                var wait = new WebDriverWait(driver, new TimeSpan(0, 0, 600));
                driver.Navigate().GoToUrl("https://os-jo.com/components/storage/solid-state-drive-ssd");
                Thread.Sleep(12000);
                ...
                while (true)
                {
                    try
                    {
                        var SSD = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                        for (int i = 0; i < SSD.Count(); i++)
                        {
                            Thread.Sleep(5000);
                            string oldPriceBeforeClick = "0";
                            if (wait.Until(driver => SSD[i].FindElements(By.CssSelector("p.price > span.price-old"))).Count > 0)
                            {
                                var oldPriceParentB = wait.Until(driver => SSD[i].FindElement(By.CssSelector("p.price > span.price-old")));
                            ...
                            var priceParentB = wait.Until(driver => SSD[i].FindElement(By.CssSelector(".price")));
                            ...
                            var NameBeforeClick = SSD[i].FindElement(By.CssSelector("div.caption > h4"));
                            ...
                            else
                            {
                                ...
                                js.ExecuteScript("arguments[0].scrollIntoView();", SSD[i]);
                                // Wait for the element to become clickable
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(SSD[i]));
                                //click on Computer
                                actions.MoveToElement(SSD[i].FindElement(By.CssSelector("div.caption > h4 > a"))).Click().Build().Perform();

                                Thread.Sleep(10000);
                                //get Computer name
                                var SSDName = wait.Until(driver => driver.FindElement(By.CssSelector(".tb_wt h1")));
                                //get Computer img
                                var SSDImage = wait.Until(driver => driver.FindElement(By.CssSelector("div.tb_slides > span > img")));
                                //get Computer price
                                string oldPrice = "0";
                                ...
                                //get SSD stock
                                var SSDStock = wait.Until(driver => driver.FindElements(By.CssSelector("span"))
                                ...
                                await _fireBaseServices.insertDataIntoFirebase(endPoint,storeName, SSDName.Text, SSDImage.GetAttribute("src"), oldPrice, newPrice, SSDStock.Text, brand, liTexts, productUrl);
                                driver.Navigate().Back();
                                // clear the list
                                liTexts.Clear();
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementIsVisible(By.CssSelector(".product-thumb")));
                                SSD = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
