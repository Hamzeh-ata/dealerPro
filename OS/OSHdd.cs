     ...
     private HashSet<string> storedproducts = new HashSet<string>();
        ...
        public async Task getHDD()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://os-jo.com/components/storage/hdd-storage");
                Thread.Sleep(12000);
                ...
                while (true)
                {
                    try
                    {
                        var HDD = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                        for (int i = 0; i < HDD.Count(); i++)
                        {
                            ...
                            else
                            {
                                ...
                                HDD = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
