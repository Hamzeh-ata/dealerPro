    ...
    public class OSRams : Hub, IOSRams
    {
        ...
        public async Task getRam()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://os-jo.com/components/system-memory");
                Thread.Sleep(12000);
                ...
                while (true)
                {
                    try
                    {
                        var Ram = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                        for (int i = 0; i < Ram.Count(); i++)
                        {
                            Thread.Sleep(5000);
                            string oldPriceBeforeClick = "0";
                            if (wait.Until(driver => Ram[i].FindElements(By.CssSelector("p.price > span.price-old"))).Count > 0)
                            {
                                var oldPriceParentB = wait.Until(driver => Ram[i].FindElement(By.CssSelector("p.price > span.price-old")));
                            ...
                            var priceParentB = wait.Until(driver => Ram[i].FindElement(By.CssSelector(".price")));
                            ...
                            var NameBeforeClick = Ram[i].FindElement(By.CssSelector("div.caption > h4"));
                            ...
                            else
                            {
                                ...
                                js.ExecuteScript("arguments[0].scrollIntoView();", Ram[i]);
                                // Wait for the element to become clickable
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(Ram[i]));
                                //click on Computer
                                actions.MoveToElement(Ram[i].FindElement(By.CssSelector("div.caption > h4 > a"))).Click().Build().Perform();

                                Thread.Sleep(10000);
                                //get Computer name
                                var RamName = wait.Until(driver => driver.FindElement(By.CssSelector(".tb_wt h1")));
                                //get Computer img
                                var RamImage = wait.Until(driver => driver.FindElement(By.CssSelector("div.tb_slides > span > img")));
                                //get Computer price
                                string oldPrice = "0";
                                ...
                                //get Computer stock
                                var RamStock = wait.Until(driver => driver.FindElements(By.CssSelector("span"))
                                ...
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, RamName.Text, RamImage.GetAttribute("src"), oldPrice, newPrice, RamStock.Text, brand, liTexts,productUrl);
                                driver.Navigate().Back();
                                // clear the list
                                liTexts.Clear();
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementIsVisible(By.CssSelector(".product-thumb")));
                                Ram = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
