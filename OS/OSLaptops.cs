    ...
    public class OSLaptops : Hub, IOSLaptops
    {
        ...
        public async Task getLaptops()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://os-jo.com/computer-systems/laptops/gaming-laptops");
                Thread.Sleep(12000);
                ...
                while (true)
                {
                    try
                    {
                        var Laptop = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                        for (int i = 0; i < Laptop.Count(); i++)
                        {
                            ...
                            else
                            {
                                ...
                                //click on Computer
                                actions.MoveToElement(Laptop[i].FindElement(By.CssSelector("div.caption > h4 > a"))).Click().Build().Perform();
                                ...
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, LaptopName.Text, LaptopImage.GetAttribute("src"), oldPrice, newPrice, LaptopStock.Text, brand, liTexts,productUrl);
                                driver.Navigate().Back();
                                // clear the list
                                liTexts.Clear();
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementIsVisible(By.CssSelector(".product-thumb")));
                                Laptop = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
