    ...
    public class OSComputers : Hub, IOSComputers
    {
        ...
        public async Task getComputers()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://os-jo.com/computer-systems/desktop-computer");
                Thread.Sleep(12000);
                ...
                while (true)
                {
                    try
                    {
                        var Computer = driver.FindElements(By.CssSelector(".product-thumb"));
                        for (int i = 0; i < Computer.Count(); i++)
                        {
                            Thread.Sleep(5000);
                            string oldPriceBeforeClick = "0";
                            if (wait.Until(driver => Computer[i].FindElements(By.CssSelector("p.price > span.price-old"))).Count > 0)
                            {
                                var oldPriceParentB = wait.Until(driver => Computer[i].FindElement(By.CssSelector("p.price > span.price-old")));
                            ...
                            var priceParentB = wait.Until(driver => Computer[i].FindElement(By.CssSelector(".price")));
                            ...
                            var NameBeforeClick = Computer[i].FindElement(By.CssSelector("div.caption > h4"));
                            ...
                            else
                            {
                                ...
                                js.ExecuteScript("arguments[0].scrollIntoView();", Computer[i]);
                                // Wait for the element to become clickable
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(Computer[i]));
                                //click on Computer
                                actions.MoveToElement(Computer[i].FindElement(By.CssSelector("div.caption > h4 > a"))).Click().Build().Perform();
                                ...
                                Computer = driver.FindElements(By.CssSelector(".product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
