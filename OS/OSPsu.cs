    ...
    public class OSPsu : Hub, IOSPsu
    {
        ...
        public async Task getPsu()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://os-jo.com/components/power-supplies");
                Thread.Sleep(12000);
                ...
                while (true)
                {
                    try
                    {
                        var PSU = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                        for (int i = 0; i < PSU.Count(); i++)
                        {
                            Thread.Sleep(5000);
                            string oldPriceBeforeClick = "0";
                            if (wait.Until(driver => PSU[i].FindElements(By.CssSelector("p.price > span.price-old"))).Count > 0)
                            {
                                var oldPriceParentB = wait.Until(driver => PSU[i].FindElement(By.CssSelector("p.price > span.price-old")));
                            ...
                            var priceParentB = wait.Until(driver => PSU[i].FindElement(By.CssSelector(".price")));
                            ...
                            var NameBeforeClick = PSU[i].FindElement(By.CssSelector("div.caption > h4"));
                            ...
                            else
                            {
                                ...
                                js.ExecuteScript("arguments[0].scrollIntoView();", PSU[i]);
                                // Wait for the element to become clickable
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(PSU[i]));
                                //click on Computer
                                actions.MoveToElement(PSU[i].FindElement(By.CssSelector("div.caption > h4 > a"))).Click().Build().Perform();
                                Thread.Sleep(5000);
                                if (driver.FindElements(By.CssSelector("#System_hKPAxwfD > .tb_text_wrap > p")).Count > 0 && driver.FindElement(By.CssSelector("#System_hKPAxwfD > .tb_text_wrap > p")).Text == "The page you requested cannot be found.")
                                {
                                    Thread.Sleep(5000);
                                    driver.Navigate().Back();
                                    wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementIsVisible(By.CssSelector(".product-thumb")));
                                    PSU = driver.FindElements(By.CssSelector(".product-thumb"));
                                }
                                else
                                {
                                    ...
                                    Thread.Sleep(10000);
                                    //get PSU name
                                    var PSUName = wait.Until(driver => driver.FindElement(By.CssSelector(".tb_wt h1")));
                                    //get PSU img
                                    var PSUImage = wait.Until(driver => driver.FindElement(By.CssSelector("div.tb_slides > span > img")));
                                    //get Computer price
                                    string oldPrice = "0";
                                    ...
                                    actions.MoveToElement(aParent).Perform();
                                    if (driver.FindElements(By.CssSelector("#Group_AYu75Txx > .tb_tabs > ul.nav-tabs li a[href='#Ua8dG_tab']")).Count>0)
                                    {                                                  
                                        Console.WriteLine("Displayed");
                                        IWebElement aElement = driver.FindElement(By.CssSelector("#Group_AYu75Txx > .tb_tabs > ul.nav-tabs li a[href='#Ua8dG_tab']"));
                                        ...
                                        else if (classValue.Contains("active"))
                                        {
                                            Thread.Sleep(5000);
                                            wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(aElement));
                                            actions.MoveToElement(aElement).Click().Build().Perform();
                                    ...
                                    else if (driver.FindElements(By.CssSelector("#ProductDescriptionSystem_Bx7ALQdc > .tb_product_description ")).Count > 0 && driver.FindElements(By.CssSelector("#Group_AYu75Txx > .tb_tabs > ul.nav-tabs li a[href='#Ua8dG_tab']")).Count<=0)
                                    {
                                    ...
                                    //get Computer stock
                                    var PSUStock = wait.Until(driver => driver.FindElements(By.CssSelector("span"))
                                    ...
                                    await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, PSUName.Text, PSUImage.GetAttribute("src"), oldPrice, newPrice, PSUStock.Text, brand, liTexts, productUrl);
                                    driver.Navigate().Back();
                                    // clear the list
                                    liTexts.Clear();
                                    wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementIsVisible(By.CssSelector(".product-thumb")));
                                    PSU = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                                }
                            }
                        }
                    }
