    ...
    public class OSGpu : Hub, IOSGpu
    {
        ...
        public async Task getGpu()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://os-jo.com/components/graphics-cards");
                Thread.Sleep(12000);
                ...
                while (true)
                {
                    try
                    {
                        var GPU = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                        for (int i = 0; i < GPU.Count(); i++)
                        {
                            ...
                            else
                            {
                                ...
                                //click on Computer
                                actions.MoveToElement(GPU[i].FindElement(By.CssSelector("div.caption > h4 > a"))).Click().Build().Perform();
                                ...
                                GPU = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
