    ...
    public class OSMonitors : Hub, IOSMonitors
    {
        ...
        public async Task GETMonitors()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://os-jo.com/components/computer-monitors");
                Thread.Sleep(12000);
                ...
                while (true)
                {
                    ...
                    {
                        Thread.Sleep(5000);
                        var monitor = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                        for (int i = 0; i < monitor.Count(); i++)
                        {
                            Thread.Sleep(5000);
                            string oldPriceBeforeClick = "0";
                            if (wait.Until(driver => monitor[i].FindElements(By.CssSelector("p.price > span.price-old"))).Count > 0)
                            {
                                var oldPriceParentB = wait.Until(driver => monitor[i].FindElement(By.CssSelector("p.price > span.price-old")));
                            ...
                            var priceParentB = wait.Until(driver => monitor[i].FindElement(By.CssSelector(".price")));
                            ...
                            var NameBeforeClick = monitor[i].FindElement(By.CssSelector("div.caption > h4"));
                            ...
                            else
                            {
                                ...
                                js.ExecuteScript("arguments[0].scrollIntoView();", monitor[i]);
                                // Wait for the element to become clickable
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(monitor[i]));
                                //click on Computer
                                actions.MoveToElement(monitor[i].FindElement(By.CssSelector("div.caption > h4 > a"))).Click().Build().Perform();
                                Thread.Sleep(5000);
                                if (driver.FindElements(By.CssSelector("#System_hKPAxwfD > .tb_text_wrap > p")).Count > 0 && driver.FindElement(By.CssSelector("#System_hKPAxwfD > .tb_text_wrap > p")).Text == "The page you requested cannot be found.")
                                {
                                    ...
                                    monitor = driver.FindElements(By.CssSelector(".product-thumb"));
                                }
                                else
                                {
                                    Thread.Sleep(10000);
                                    //get monitor name
                                    var monitorName = wait.Until(driver => driver.FindElement(By.CssSelector(".tb_wt h1")));
                                    //get monitor img
                                    var monitorImage = wait.Until(driver => driver.FindElement(By.CssSelector("div.tb_slides > span > img")));
                                    //get mb price
                                    string oldPrice = "0";
                                    ...
                                    //get monitor stock
                                    var monitorStock = wait.Until(driver => driver.FindElements(By.CssSelector("span"))
                                    ...
                                    await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, monitorName.Text, monitorImage.GetAttribute("src"), oldPrice, newPrice, monitorStock.Text, brand, liTexts, productUrl);

                                    driver.Navigate().Back();

                                    liTexts.Clear();
                                    wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementIsVisible(By.CssSelector(".product-thumb")));
                                    monitor = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
