    ...
    public class OSHeadset : Hub, IOSHeadset
    {
        ...
        public async Task GetHeadSet()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://os-jo.com/accessories/audio/headset");
                Thread.Sleep(12000);
                ...
                while (true)
                {
                    ...
                    {
                        Thread.Sleep(5000);
                        var HeadSet = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
                        for (int i = 0; i < HeadSet.Count(); i++)
                        {
                            ...
                            else
                            {
                                ...
                                //click on Computer
                                actions.MoveToElement(HeadSet[i].FindElement(By.CssSelector("div.caption > h4 > a"))).Click().Build().Perform();
                                ...
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, HeadSetName.Text, HeadSetImage.GetAttribute("src"), oldPrice, newPrice, HeadSetStock.Text, brand, liTexts, productUrl);

                                driver.Navigate().Back();

                                liTexts.Clear();
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementIsVisible(By.CssSelector(".product-thumb")));
                                HeadSet = driver.FindElements(By.CssSelector("#ProductsSystem_YD9pMDOx .product-thumb"));
