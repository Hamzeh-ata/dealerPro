      ...
      //      chromeOptions.AddArgument("--no-sandbox");
        //    chromeOptions.AddArgument("--disable-dev-shm-usage");
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://gts.jo/gaming/gaming-headset");
                Thread.Sleep(10000);
                ...
                while (true)
                {
                    try
                    {
                        var Headset = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                        for (int i = 0; i < Headset.Count(); i++)
                        {
                            Thread.Sleep(5000);
                            string oldPriceBeforeClick = "0";
                            if (wait.Until(driver => Headset[i].FindElements(By.CssSelector("p.price > span.price-old"))).Count > 0)
                            {
                                var oldPriceParentB = wait.Until(driver => Headset[i].FindElement(By.CssSelector("p.price > span.price-old")));
                            ...
                            var priceParentB = wait.Until(driver => Headset[i].FindElement(By.CssSelector(".price")));
                            ...
                            var NameBeforeClick = Headset[i].FindElement(By.CssSelector("div.caption > h4"));
                            ...
                            else
                            {
                                ...
                                js.ExecuteScript("arguments[0].scrollIntoView();", Headset[i]);
                                // Wait for the element to become clickable
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(Headset[i]));
                                //click on Headset
                                actions.MoveToElement(Headset[i].FindElement(By.CssSelector("div.caption > h4"))).Click().Build().Perform(); Thread.Sleep(10000);
                                //get Headset name
                                var HeadsetName = wait.Until(driver => driver.FindElement(By.CssSelector(".tb_wt h1")));
                                //get Headset img
                                var HeadsetImage = wait.Until(driver => driver.FindElement(By.CssSelector("div.tb_slides > span > img")));
                                //get Headset price
                                string oldPrice = "0";
                                ...
                                //get Headset stock
                                var HeadsetStock = wait.Until(driver => driver.FindElements(By.CssSelector("span"))
                                ...
                                //insert Headset into firebase
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, HeadsetName.Text, HeadsetImage.GetAttribute("src"), oldPrice, newPrice, HeadsetStock.Text, brand.Text, liTexts,productUrl);
                                storedproducts.Add(HeadsetName.Text);
                                ...
                                Headset = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
