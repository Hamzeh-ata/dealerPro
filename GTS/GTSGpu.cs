        ...
        public async Task getGPU()
        {
           ...
           chromeOptions.AddArgument("--no-sandbox");
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://gts.jo/components/computer-components/graphic-cards");
                Thread.Sleep(10000);
                ...
                while (true)
                {
                    try
                    {
                        ...
                        for (int i = 0; i < GPU.Count(); i++)
                        {
                            ...
                            if (wait.Until(driver => GPU[i].FindElements(By.CssSelector("p.price > span.price-old"))).Count > 0)
                            {
                                var oldPriceParentB = wait.Until(driver => GPU[i].FindElement(By.CssSelector("p.price > span.price-old")));
                            ...
                            var priceParentB = wait.Until(driver => GPU[i].FindElement(By.CssSelector(".price")));
                            ...
                            var NameBeforeClick = GPU[i].FindElement(By.CssSelector("div.caption > h4"));
                            ...
                            else
                            {
                                ...
                                js.ExecuteScript("arguments[0].scrollIntoView();", GPU[i]);
                                // Wait for the element to become clickable
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(GPU[i]));
                                //click on GPU
                                actions.MoveToElement(GPU[i].FindElement(By.CssSelector("div.caption > h4"))).Click().Build().Perform();
                                Thread.Sleep(10000);
                                //get GPU name
                                ...
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, GPUName.Text, GPUImage.GetAttribute("src"), oldPrice, price, GPUStock.Text, brand.Text, liTexts,productUrl);
                                ...
                                GPU = driver.FindElements(By.CssSelector(".product-layout > .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
