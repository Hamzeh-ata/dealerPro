         ...
         //   chromeOptions.AddArgument("--no-sandbox");
           // chromeOptions.AddArgument("--disable-dev-shm-usage");
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://gts.jo/components/ram");
                Thread.Sleep(10000);
                ...
                while (true)
                {
                    try
                    {
                        var RAM = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                        for (int i = 0; i < RAM.Count(); i++)
                        {
                            ...
                            else
                            {
                                ...
                                //click on RAM
                                actions.MoveToElement(RAM[i].FindElement(By.CssSelector("div.caption > h4"))).Click().Build().Perform();
                                ...
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, RAMName.Text, RAMImage.GetAttribute("src"), oldPrice, price, RAMStock.Text, brand.Text, liTexts, productUrl);
                                storedproducts.Add(RAMName.Text);
                                ...
                                RAM = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
