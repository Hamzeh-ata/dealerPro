        ...
        public async Task getHDD()
        {
         ...
         //   chromeOptions.AddArgument("--disable-dev-shm-usage");
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://gts.jo/components/internal-storage/hdd-internal-storage");
                Thread.Sleep(10000);
                ...
                while (true)
                {
                    try
                    {
                        var HDD = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                        for (int i = 0; i < HDD.Count(); i++)
                        {
                            ...
                            else 
                            {
                                ...
                                //click on HDD
                                actions.MoveToElement(HDD[i].FindElement(By.CssSelector("div.caption > h4"))).Click().Build().Perform();
                                Thread.Sleep(10000);
                                //get HDD name
                                ...
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, HDDName.Text, HDDImage.GetAttribute("src"), oldPrice, price, HDDStock.Text, brand.Text, liTexts,productUrl);
                                storedproducts.Add(HDDName.Text);
                                ...
                                HDD = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
