        ...
        public async Task getMonitor()
        {
         ...
         //   chromeOptions.AddArgument("--disable-dev-shm-usage");
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://gts.jo/gaming/gaming-monitor");
                Thread.Sleep(10000);
                ...
                while (true)
                {
                    try
                    {
                        var Monitor = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                        for (int i = 0; i < Monitor.Count(); i++)
                        {
                            ...
                            else
                            {
                                ...
                                //click on Monitor
                                actions.MoveToElement(Monitor[i].FindElement(By.CssSelector("div.caption > h4"))).Click().Build().Perform();
                                Thread.Sleep(10000);
                                //get Monitor name
                                ...
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, MonitorName.Text, MonitorImage.GetAttribute("src"), oldPrice, newPrice, MonitorStock.Text, brand.Text, liTexts, productUrl);
                                ...
                                Monitor = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
