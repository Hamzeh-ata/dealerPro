         ...
         //   chromeOptions.AddArgument("--no-sandbox");
          //  chromeOptions.AddArgument("--disable-dev-shm-usage");
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://gts.jo/components/computer-components/power-supplies");
                Thread.Sleep(10000);
                ...
                while (true)
                {
                    try
                    {
                        var PSU = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                        for (int i = 0; i < PSU.Count(); i++)
                        {
                            ...
                            else
                            {
                                ...
                                //click on PSU
                                actions.MoveToElement(PSU[i].FindElement(By.CssSelector("div.caption > h4"))).Click().Build().Perform();
                                Thread.Sleep(10000);
                                //get PSU name
                                ...
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, PSUName.Text, PSUImage.GetAttribute("src"), oldPrice, price, PSUStock.Text, brand.Text, liTexts, productUrl);
                                storedproducts.Add(PSUName.Text);
                                ...
                                PSU = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
