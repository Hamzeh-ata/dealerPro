         ...
         //   chromeOptions.AddArgument("--disable-gpu");
          ...
          //  chromeOptions.AddArgument("--disable-dev-shm-usage");
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://gts.jo/gaming/gaming-keyboard");
                Thread.Sleep(10000);
                ...
                while (true)
                {
                    try
                    {
                        var Keyboard = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                        for (int i = 0; i < Keyboard.Count(); i++)
                        {
                            ...
                            else
                            {
                                ...
                                //click on Keyboard
                                actions.MoveToElement(Keyboard[i].FindElement(By.CssSelector("div.caption > h4"))).Click().Build().Perform(); Thread.Sleep(10000);
                                ...
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, KeyboardName.Text, KeyboardImage.GetAttribute("src"), oldPrice, newPrice, KeyboardStock.Text, brand.Text, liTexts,productUrl);
                                ...
                                Keyboard = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
