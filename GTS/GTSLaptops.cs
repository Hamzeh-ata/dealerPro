        ...
        public async Task getLaptops()
        {
         ...
         //  chromeOptions.AddArgument("--disable-dev-shm-usage");
            ...
            using(var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://gts.jo/computers/laptops/gaming-laptop");
                Thread.Sleep(10000);
                ...
                while (true)
                {
                    try
                    {
                        var laptop = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                        for (int i = 0; i < laptop.Count(); i++)
                        {
                            Thread.Sleep(5000);
                            string oldPriceBeforeClick = "0";
                            if (wait.Until(driver => laptop[i].FindElements(By.CssSelector("p.price > span.price-old"))).Count > 0)
                            {
                                var oldPriceParentB = wait.Until(driver => laptop[i].FindElement(By.CssSelector("p.price > span.price-old")));
                            ...
                            var priceParentB = wait.Until(driver => laptop[i].FindElement(By.CssSelector(".price")));
                            ...
                            var NameBeforeClick = laptop[i].FindElement(By.CssSelector("div.caption > h4"));
                            ...
                            else
                            {
                                ...
                                js.ExecuteScript("arguments[0].scrollIntoView();", laptop[i]);
                                // Wait for the element to become clickable
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(laptop[i]));
                                //click on laptop
                                actions.MoveToElement(laptop[i].FindElement(By.CssSelector("div.caption > h4"))).Click().Build().Perform(); 
                                Thread.Sleep(10000);
                                //get laptop name
                                var laptopName = wait.Until(driver => driver.FindElement(By.CssSelector(".tb_wt h1")));
                                //get laptop img
                                var laptopImage = wait.Until(driver => driver.FindElement(By.CssSelector("div.tb_slides > span > img")));
                                //get laptop price
                                ...
                                //get laptop stock
                                var laptopStock = wait.Until(driver => driver.FindElements(By.CssSelector("span"))
                                ...
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, laptopName.Text, laptopImage.GetAttribute("src"), oldPrice, price, laptopStock.Text, brand.Text, liTexts, productUrl);
                                ...
                                laptop = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
