    ...
    public class GTSMouse : Hub, IGTSMouse
    {
        ...
        public async Task getMouse()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://gts.jo/accessories/laptop-desktop-accessories/mouse-1");
                Thread.Sleep(10000);
                ...
                while (true)
                {
                    try
                    {
                        var Mouse = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                        for (int i = 0; i < Mouse.Count(); i++)
                        {
                            ...
                            else
                            {
                                ...
                                //click on Mouse
                                actions.MoveToElement(Mouse[i].FindElement(By.CssSelector("div.caption > h4"))).Click().Build().Perform();
                                Thread.Sleep(10000);
                                //get Mouse name
                                ...
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, MouseName.Text, MouseImage.GetAttribute("src"), oldPrice, newPrice, MouseStock.Text, brand.Text, liTexts, productUrl);
                                ...
                                Mouse = driver.FindElements(By.CssSelector(".tb_wt > .tb_products > .product-layout > .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
