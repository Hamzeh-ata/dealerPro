    ...
    public class smartBuyRefrigerators : Hub, IsmartBuyRefrigerators
    {
        ...
        public async Task getRefrigerator()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://smartbuy-me.com/smartbuystore/ar/%D8%A7%D9%84%D8%A7%D8%AC%D9%87%D8%B2%D8%A9-%D8%A7%D9%84%D9%85%D9%86%D8%B2%D9%84%D9%8A%D8%A9/%D8%A7%D9%84%D8%A7%D8%AC%D9%87%D8%B2%D8%A9-%D8%A7%D9%84%D9%83%D8%A8%D9%8A%D8%B1%D8%A9/%D8%AB%D9%84%D8%A7%D8%AC%D8%A7%D8%AA/c/30108");
                Thread.Sleep(5000);
                Actions actions = new Actions(driver);
                bool flag = true;
                var url = "https://smartbuy-me.com/smartbuystore/ar/%D8%A7%D9%84%D8%A7%D8%AC%D9%87%D8%B2%D8%A9-%D8%A7%D9%84%D9%85%D9%86%D8%B2%D9%84%D9%8A%D8%A9/%D8%A7%D9%84%D8%A7%D8%AC%D9%87%D8%B2%D8%A9-%D8%A7%D9%84%D9%83%D8%A8%D9%8A%D8%B1%D8%A9/%D8%AB%D9%84%D8%A7%D8%AC%D8%A7%D8%AA/c/30108";
                while (true)
                {
                    Thread.Sleep(2000);
                ...
                while (flag)
                {
                    try
                    {
                        ...
                        Thread.Sleep(5000);
                        var Refrigerator = wait.Until(driver => driver.FindElements(By.CssSelector(".product-item")));
                        int initialRefrigeratorsCount = Refrigerator.Count;
                        // Console.WriteLine(driver.FindElements(By.CssSelector(".product-item")).ToList().Count);
                        //loop through all mobiles using productList
                        for (int i = 0; i < initialRefrigeratorsCount; i++)
                        {

                            Console.WriteLine("number of Refrigerators " + initialRefrigeratorsCount);
                            Thread.Sleep(2000);
                            // actions.MoveToElement(productList[i]).Perform();
                            js.ExecuteScript("arguments[0].scrollIntoView();", Refrigerator[i]);
                            string oldPriceBeforeClick = "0";
                            string priceBeforeClick = "0";
                            if (wait.Until(driver => Refrigerator[i].FindElement(By.CssSelector("span.discountPrice"))).Displayed)
                            {
                                var oldPriceSpan = wait.Until(driver => Refrigerator[i].FindElement(By.CssSelector("span.discountPrice")));
                                oldPriceBeforeClick = oldPriceSpan.Text;
                                priceBeforeClick = wait.Until(driver => Refrigerator[i].FindElement(By.CssSelector("span.orignalPrice"))).Text;
                                Console.WriteLine("oldPrice");
                            }
                            else
                            {
                                priceBeforeClick = wait.Until(driver => Refrigerator[i].FindElement(By.CssSelector("div.price"))).Text;
                                oldPriceBeforeClick = "0";
                                Console.WriteLine("no oldPrice");
                            }

                            var NameBeforeClick = wait.Until(driver => Refrigerator[i].FindElement(By.CssSelector("a.name.hidden-lg")));
                            ...
                            else
                            {
                                ...
                                driver.ExecuteScript("window.open(arguments[0]);", url + Refrigerator[i].FindElement(By.CssSelector(".thumb")).GetAttribute("href"));
                            ...
                            if (i >= initialRefrigeratorsCount - 1)
                            {
