    ...
    public class cityCenterPCs : Hub, IcityCenterPCs
    {
        ...
        public async Task GetPC()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://citycenter.jo/gaming/gaming-gaming-pcs");
                ...
                while (true)
                {
                    ...
                    {
                        Thread.Sleep(5000);
                        var PCs = wait.Until(driver => driver.FindElements(By.CssSelector(".product-layout > .product-thumb")));
                        for (int i = 0; i < PCs.Count(); i++) // loop through all CPUs except the last one
                        {

                            Thread.Sleep(5000);
                            string oldPriceBeforeClick = "0";
                            if (wait.Until(driver => PCs[i].FindElements(By.CssSelector("p.price > span.price-old"))).Count > 0)
                            {
                                var oldPriceParentB = wait.Until(driver => PCs[i].FindElement(By.CssSelector("p.price > span.price-old")));
                            ...
                            var priceParentB = wait.Until(driver => PCs[i].FindElement(By.CssSelector(".price")));
                            ...
                            var NameBeforeClick = PCs[i].FindElement(By.CssSelector("div.caption > h4"));
                            ...
                            else
                            {
                                ...
                                js.ExecuteScript("arguments[0].scrollIntoView();", PCs[i]);
                                // Wait for the element to become clickable
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(PCs[i]));
                                //click on PC
                                PCs[i].Click();
                                //get PC name
                                Thread.Sleep(10000);
                                ...
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, PCName.Text, PCImage.GetAttribute("src"), oldPrice, newPrice, PCStock.Text, brand, liTexts, productUrl);
                                ...
                                PCs = driver.FindElements(By.CssSelector(".product-layout > .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
