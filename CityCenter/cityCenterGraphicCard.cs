    ...
    public class cityCenterGraphicCard : Hub, IcityCenterGraphicCard
    {
        ...
        public async Task GetGraphicCard()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://citycenter.jo/gaming/gaming-graphic-card");
                ...
                while (true)
                {
                    ...
                    {
                        Thread.Sleep(10000);
                        var linkElement = wait.Until(driver => driver.FindElements(By.CssSelector(".product-layout > .product-thumb")));
                        //var GPU = driver.FindElements(By.CssSelector(".product-layout > .product-thumb"));
                        for (int i = 0; i < linkElement.Count(); i++) // loop through all CPUs except the last one
                        {
                            Thread.Sleep(5000);
                            string oldPriceBeforeClick = "0";
                            if (wait.Until(driver => linkElement[i].FindElements(By.CssSelector("p.price > span.price-old"))).Count > 0)
                            {
                                var oldPriceParentB = wait.Until(driver => linkElement[i].FindElement(By.CssSelector("p.price > span.price-old")));
                            ...
                            var priceParentB = wait.Until(driver => linkElement[i].FindElement(By.CssSelector(".price")));
                            ...
                            var NameBeforeClick = linkElement[i].FindElement(By.CssSelector("div.caption > h4"));
                            ...
                            else
                            {
                                ...
                                js.ExecuteScript("arguments[0].scrollIntoView();", linkElement[i]);
                                // Wait for the element to become clickable
                                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(linkElement[i]));
                                //click on GPU
                                actions.MoveToElement(linkElement[i].FindElement(By.CssSelector("div.caption > h4 > a"))).Click().Build().Perform();
                                
                                //get GPU name
                                var gpuName = wait.Until(driver => driver.FindElement(By.CssSelector(".tb_wt h1")));
                                //get GPU img
                                var gpuImage = wait.Until(driver => driver.FindElement(By.CssSelector("div.tb_slides > span > img")));
                                //get GPU price
                                string oldPrice = "0";
                                ...
                                //get GPU stock
                                var gpuStock = wait.Until(driver => driver.FindElements(By.CssSelector("span"))
                                ...
                                //insert gpu into firebase
                                string productUrl = driver.Url;
                                await _fireBaseServices.insertDataIntoFirebase(endPoint, storeName, gpuName.Text, gpuImage.GetAttribute("src"), oldPrice, newPrice, gpuStock.Text, brand, liTexts, productUrl);
                                storedproducts.Add(gpuName.Text);
                                ...
                                linkElement = driver.FindElements(By.CssSelector(".product-layout > .product-thumb"));
                            }
                        }
                    }
                    catch (NoSuchElementException ex)
