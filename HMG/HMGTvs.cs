    ...
    public class HMGTvs : Hub, IHMGTvs
    {
        ...
        public async Task getTv()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://hmg.jo/product-category/audio-visual/");
                Thread.Sleep(12000);
                ...
                try
                {
                    if(driver.FindElement(By.CssSelector("div[style*='cursor: pointer'][style*='right: -4px']")).Displayed)
                    {
                        Console.WriteLine("pop");
                        IWebElement popUp = driver.FindElement(By.CssSelector("div[style*='cursor: pointer'][style*='right: -4px']"));
                ...
                while (flag)
                {
                    ...
                    {
                        Thread.Sleep(5000);
                        var tv = driver.FindElements(By.CssSelector(".site-main > ul.products li.product > .product-item__outer > .product-item__inner"));
                        int tvCount = tv.Count;
                        for (int i = 0; i < tv.Count(); i++)
                        {
                            ...
                            ((IJavaScriptExecutor)driver).ExecuteScript("arguments[0].scrollIntoView({behavior: 'auto', block: 'center', inline: 'center'});", tv[i]);
                            // Wait for the element to become clickable
                            Thread.Sleep(5000);
                            actions.MoveToElement(tv[i].FindElement(By.CssSelector(".woocommerce-loop-product__link"))).Click().Build().Perform();
                            Thread.Sleep(10000);
                            try
                            {
                                if (driver.FindElement(By.CssSelector("div[style*='cursor: pointer'][style*='right: -4px']")).Displayed)
                            ...
                            tv = driver.FindElements(By.CssSelector(".site-main > ul.products li.product > .product-item__outer > .product-item__inner"));
