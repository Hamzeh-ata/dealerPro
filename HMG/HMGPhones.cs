    ...
    public class HMGPhones : Hub, IHMGPhones
    {
        ...
        public async Task getPhone()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://hmg.jo/product-category/electronics/mobile-phones/");
                Thread.Sleep(12000);
                ...
                while (flag)
                {
                    ...
                    {
                        Thread.Sleep(5000);
                        var Phones = driver.FindElements(By.CssSelector(".site-main > ul.products li.product > .product-item__outer > .product-item__inner"));
                        ...
                        for (int i = 0; i < PhonesCount; i++)
                        {
                            ...
                            wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(Phones[i]));

                            //  open the link in a new tab using JavaScript
                            driver.ExecuteScript("window.open(arguments[0]);", Phones[i].FindElement(By.CssSelector("a.woocommerce-LoopProduct-link")).GetAttribute("href"));
