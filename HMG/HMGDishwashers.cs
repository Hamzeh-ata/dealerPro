    ...
    public class HMGDishwashers : Hub, IHMGDishwashers
    {
        ...
        public async Task getGDishwashers()
        {
            ...
            using (var driver = new ChromeDriver(service, chromeOptions))
            {
                ...
                driver.Navigate().GoToUrl("https://hmg.jo/product-category/dishwashers/");
                Thread.Sleep(12000);
                ...
                while (flag)
                {
                    ...
                    {
                        Thread.Sleep(5000);
                        var Dishwashers = driver.FindElements(By.CssSelector(".site-main > ul.products li.product > .product-item__outer > .product-item__inner"));
                        int DishwashersCount = Dishwashers.Count();
                        for (int i = 0; i < DishwashersCount; i++)
                        {
                            Thread.Sleep(5000);
                            Console.WriteLine("the number of all products is " + DishwashersCount + "The number of i is " + i);
                            // Scroll the element into view using JavaScript
                            wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(Dishwashers[i]));
                            IJavaScriptExecutor js = (IJavaScriptExecutor)driver;
                            js.ExecuteScript("arguments[0].scrollIntoView();", Dishwashers[i]);
                            //  open the link in a new tab using JavaScript
                            driver.ExecuteScript("window.open(arguments[0]);", Dishwashers[i].FindElement(By.CssSelector("a.woocommerce-LoopProduct-link")).GetAttribute("href"));
                            ...
                            if (i >= DishwashersCount - 1)
                            {
                                 await _fireBaseServices.HMGcheckIfElementIsStillExists(endPoint,storeName,storedproducts);
